...
sealed trait MergingQueue[+T] {
  ...
  final def append[B >: T](element: B): MergingQueue[B] =
    this match {
  ...
  final def remove[B >: T](element: B): MergingQueue[B] =
    this match {
      case Nobody                                             => Nobody
  ...
  final def concat[B >: T](mergingQueue: MergingQueue[B]): MergingQueue[B] =
    mergingQueue match {
      case Nobody                  => this
  ...
  final def size: Int = {
    def recursiveSize(queue: MergingQueue[T], acc: Int): Int =
      queue match {
        case Nobody               => acc
...
object MergingQueue {
  ...
  implicit def showQueue[T: Show]: Show[MergingQueue[T]] =
    Show.show { queue =>
      queue.toList match {
